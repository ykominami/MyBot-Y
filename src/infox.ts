...
import {
    StringOrNull, MultiArrayStringType, ItemxType, ItemxArrayType,
    InfoParamType, SearchItemType
} from "./@types/index.d"
    ...
    getValues(): MultiArrayStringType {
        ...
        if (this.values.length > 0) {
            ...
            if (this.values[0].length == 0) {
                ...
                // this.values = this.ssheet.getValues() as MultiArrayStringType;
                this.values = this.ssheet.getValues(); // as MultiArrayStringType;
            }
        }
        return this.values;
    }
    getSSId(infoparam: InfoParamType): StringOrNull {
        ...
        const item: SearchItemType = this.make_item(
            year,
            kind,
            kind2);
        // Util.log(`Infox getSSId item=${item}`)
        const ss_id = this.get_id_from_values(values, item);
        // Util.log(`Infox getSSId ss_id=${ss_id}`)
        return ss_id;
    }
    make_item(year_str: StringOrNull = null,
        kind_str: StringOrNull = null,
        kind2_str: StringOrNull = null): SearchItemType {
        ...
        const INDEX_URL: number = 5

        const value_item = { index: INDEX_ID, value: null };
        const search_items: ItemxArrayType = [];
        if (year_str != null) {
            const s1_item: ItemxType = { index: INDEX_YEAR, value: year_str };
            search_items.push(s1_item);
        }
        if (kind_str != null) {
            const s2_item: ItemxType = { index: INDEX_KIND, value: kind_str };
            search_items.push(s2_item);
        }
        if (kind2_str != null) {
            const s3_item: ItemxType = { index: INDEX_KIND2, value: kind2_str };
            search_items.push(s3_item);
        }

        return { searches: search_items, value: value_item };
    }
    get_id_from_values(d: MultiArrayStringType, item: SearchItemType): StringOrNull {
        let result_start: MultiArrayStringType = d
        let result_end: MultiArrayStringType = []
        let ret_str: StringOrNull = null;
        let xstr: string = "";
        ...
        for (let i = 0; i < count; i++) {
            ...
            xstr = item.value == null ? "null" : "not null";
            ...
            result_end = result_start.filter((v) => {
                return v[item.index] == item.value
            })
