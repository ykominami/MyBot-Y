...
// import { ThreeItemsAssocArray, DataRangex, ThreeItemsOrNull, StringOrNull } from "./settingsheet2";
import {
  ThreeItemsAssocArray, StringOrNull, ThreeItemsOrNull, DataRangex
} from "./@types/index.d"
...
export class SettingSheet2 {
  ...
  make_sub() {
    ...
    if (this.data != null) {
      if (x != null) {
        if (y != null) {
          this.dynalistUrl = (this.data[x][y] as string);
          // Util.log(
          //  `SettingSheet2 x=${x} y=${y} data[x][y]=${(this.data[x][y] as string)}`
          // );
        }
        if (bmY != null) {
          this.bmId = (this.data[x][bmY] as string);
