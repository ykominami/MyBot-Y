...
import { GASHtmlTextOutputType, StringOrNull, TransferData, InfoParamType } from "./@types/index.d"
...
export class Webapp {
    ...
    mainx(ss_id: string, message: string): void {
        let xstr = "";
        const json_root: TransferData = JSON.parse(message) as TransferData;
        const sheetName: string = json_root.sheetName;
        const json = json_root.data;
        const sheet = this.getOrCreateSheet(ss_id, sheetName);
        if (sheet != null) {
            const lastRow = sheet.getLastRow() + 1
            // const array: string[][] = Object.keys(json).map((key) => [key, json[key]])
            const array = Object.keys(json).map(
                (key) =>
                    [key, json[key]]
            ) as [[string, string]]
    ...
    do_post(e: GoogleAppsScript.Events.DoPost): GASHtmlTextOutputType {
        ...
        const sheetName = "ss4";
        let ss_id: StringOrNull = this.getSsidFromBaseSS(sheetName);
        if (ss_id === null) {
            ss_id = "";
        }
        this.mainx(ss_id, postContent)
    ...
    // GETリクエストに対する処理
    do_x_0(param: InfoParamType, kind2: StringOrNull, format: StringOrNull, cmd: StringOrNull): GASHtmlTextOutputType {
    ...
    testData(): GASHtmlTextOutputType {
